...
//	************************************************************
	...
	function is_sane_dir ($path)
	{
		if($this->CLEAR) { $this->clear_error(); }

		if (ereg('.tmp', $path))	 //Check tmp file for read/write capabilities
		{
			if (!($f = @fopen($path, 'w+')))
			{
				$this->ERROR = "Dir is not writeable";
				return false;
			}
			else
			{
		...
		else 
		{
			...
			if (!is_readable($path))
			{
				$this->ERROR = "Dir is not readable";
				return false;
			}
			...
			// Double Check (ie, for Windows Systems)
			if ($path{strlen($path)-1}=='/') //Start function again with tmp file...
			{
				return $this->is_sane_dir($path.uniqid(mt_rand()).'.tmp');
			}
		}
